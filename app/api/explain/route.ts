...
export async function POST(request: NextRequest) {
  try {
    ...
    if (apiKey) {
      try {
        const result = await callGemini(code, language, targetLanguage, apiKey);
        return Response.json(result);
      } catch (err: unknown) {
        const errorMessage = err instanceof Error ? err.message : String(err);
        console.error("[ByteMind API] Gemini Error, falling back to simulation:", errorMessage);
        // Fallback to simulation if Gemini fails/rate limits
